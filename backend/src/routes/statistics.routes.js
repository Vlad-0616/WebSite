...
// GET /api/statistics/popular-routes - Получение популярных маршрутов
router.get('/popular-routes', async (req, res) => {
  try {
    // Количество маршрутов в результате (по умолчанию 5)
    const limit = parseInt(req.query.limit) || 5
    // Получаем последние 100 заказов для анализа маршрутов
    ...
    // Группируем маршруты и считаем количество повторений
    const routeCounts = {}
    routes.forEach(route => {
      const key = `${route.pickup_address} → ${route.delivery_address}`
      
      if (!routeCounts[key]) {
        // Берем только название города (до запятой)
        routeCounts[key] = {
    ...
    // Сортируем по популярности (убывание) и берем топ
