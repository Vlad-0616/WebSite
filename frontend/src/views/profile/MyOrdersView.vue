        ...
        <div class="flex items-start justify-between">
          ...
          <div class="flex gap-2 ml-4">
            ...
            <button 
              ...
              class="px-4 py-2 bg-red-500/20 hover:bg-red-500/30 text-red-300 rounded-lg transition-colors text-sm"
            >
              Отменить
            </button>
          </div>
...
const fetchMyOrders = async () => {
  loading.value = true
  try {
    // Бэкенд возвращает массив напрямую, а не объект с полем data
    const response = await apiClient.get('/api/orders/my')
    orders.value = response.data  // response.data — это уже массив заказов
  } catch (error) {
