<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20 mb-8">
      ...
      <div v-show="showFilters" class="mt-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="block text-gray-300 mb-2"> Грузоподъёмность от (кг)</label>
            <input 
          ...
          <div>
            <label class="block text-gray-300 mb-2"> Грузоподъёмность до (кг)</label>
            <input 
          ...
          <div>
            <label class="block text-gray-300 mb-2"> Объём от (м³)</label>
            <input 
          ...
          <div>
            <label class="block text-gray-300 mb-2"> Объём до (м³)</label>
            <input 
    ...
    <div v-else-if="trucks.length === 0" class="text-center py-16 bg-white/10 backdrop-blur-lg rounded-xl border border-white/20">
      ...
      <p class="text-gray-400 text-lg">Нет транспорта, соответствующего фильтрам</p>
      <button 
        @click="resetFilters"
        class="mt-4 bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg transition-colors"
      >
        Сбросить фильтры
      </button>
    </div>
      ...
      >
        ...
        <div class="text-gray-300 text-sm space-y-2 mb-4">
          ...
          <div v-if="truck.plate_number" class="flex justify-between">
            <span>Госномер:</span>
            <span class="font-mono font-semibold">{{ truck.plate_number }}</span>
          </div>
          <div v-if="truck.brand || truck.model" class="flex justify-between">
            <span>Модель:</span>
            <span class="font-semibold">{{ truck.brand }} {{ truck.model }}</span>
          </div>
          <div v-if="truck.year" class="flex justify-between">
            <span>Год выпуска:</span>
            <span class="font-semibold">{{ truck.year }}</span>
          </div>
...
const fetchTrucks = async () => {
  ...
  try {
    ...
    console.log(' Запрос к /api/trucks:', params)
